...
The HTML writer ensures output safety through:

1. HTML escaping of text content using `escape_html()`
2. HTML escaping of attribute values using `escape_html()`
3. URL escaping of link destinations using `escape_href()`
4. Proper opening/closing tag matching
5. Validation of tag nesting

The escaping functions live in the `pulldown-cmark-escape` crate.
